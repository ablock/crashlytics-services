...
describe Service::Sprintly do
	...
	describe 'receive_verification' do
		before do
			@config = { :dashboard_url => 'https://sprint.ly/product/1/' }
			@service = Service::Sprintly.new('verification', {})
			@payload = {}
		end
	...
	describe 'receive_issue_impact_change' do
		before do
			@config = { :dashboard_url => 'https://sprint.ly/product/1/' }
			@service = Service::Sprintly.new('issue_impact_change', {})
			@payload = {
					:title => 'foo title',
					:impact_level => 1,
		...
		it 'should succeed upon successful api response' do
			test = Faraday.new do |builder|
				builder.adapter :test do |stub|
					stub.post('/api/products/1/items.json') { [200, {}, "{\"number\":\"42\"}"] }
					stub.get('/api/products/1/items.json') { [200, {}, ""] }
				end
			end
